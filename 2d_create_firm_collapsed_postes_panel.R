...
# Add industry
reg <- feols(data = dat, l_hwr_agg_w2 ~ 1 | year + age_bin + sexe + exp_cs_bin + pcs + a38)
dat[!is.na(l_hwr_agg_w2), l_hwr_res4 := reg$residuals]

# Add individual fixed-effect
reg <- feols(data = dat, l_hwr_agg_w2 ~ 1 | ident_all + year + age_bin + exp_cs_bin + pcs + a38)
...
dat <- dat[, .(n_cdd = sum(cdd), n = .N,
               ...
               m_lhw_res5_cdd = mean(l_hwr_res5[cdd == TRUE], na.rm = TRUE)), by = .(siren, year)]

write.fst(dat, here("Data", "ss_panel_1014_collapseSiren_hiring_wage.fst"))
